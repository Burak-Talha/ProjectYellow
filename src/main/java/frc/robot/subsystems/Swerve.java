...
public class Swerve extends SubsystemBase {
    ...
    public Pose2d getHumanPlayerTarget(){
        if(DriverStation.getAlliance().get()==Alliance.Red){
            return swerveDrivePoseEstimator.getEstimatedPosition().nearest(Arrays.asList(FieldConstants.RED_HUMAN_PLAYER_TARGETS));
        }
        return swerveDrivePoseEstimator.getEstimatedPosition().nearest(Arrays.asList(FieldConstants.BLUE_HUMAN_PLAYER_TARGETS));
    }

    public Pose2d[] getHumanPlayerABTarget(){
        if(DriverStation.getAlliance().get()==Alliance.Red){
            return FieldConstants.RED_HUMAN_PLAYER_TARGETS;
        }
        return FieldConstants.BLUE_HUMAN_PLAYER_TARGETS;   
    }
